...
Point2f clickCoords = Point2f(300,1080/2);
...
vector<Vec4f> getLines()
{
    ...
    //inRange(HSV, Scalar(35, 30, 0), Scalar(80, 219, 197), thresh);
    ...
    if(useMask){
        ...
        mask = Mat::ones( thresh.rows, thresh.cols, CV_8U);
    ...
    vector<Vec4f> lines;
    HoughLinesP(dst, lines, rho, CV_PI/360, vote, minLen, 45 );
...
vector<Vec4f> cleanLines(vector<Vec4f> sortedLines, Mat labels){
    ...
    for(int i = 0; i < clusterCount; i++){
        ...
        line( clustered, Point(pushLine[0], pushLine[1]), Point(pushLine[2], pushLine[3]), Scalar(0,128,255), 10, 0);
    }
...
Mat newClustering(Mat in, vector<Vec4f>& lines){
    ...
    for(int i = 0; i < verticals.size(); i++){
        ...
        //line(in, Point(verticals[i][0], verticals[i][1]), Point(verticals[i][2], verticals[i][3]), colour, 5);
        labels.push_back(cluster);
        lastX = x;
    }
    
    showCluster = in.clone();
    clustered = in.clone();
    //imshow("clustered", in);
    //waitKey();
    ...
    if(!src.empty()){
        ...
        vector<Vec4f> templateLines;
         ...
         if(selectedLine == 0 ){ // left
             ...
             //cout << "matching rightmost line " << endl;
             int closest = -1;
             int closestDist = -1;
             for(int i = 0; i < lines.size(); i++){
                 if(minimum_distance(lines[i], clickCoords) < closestDist || closestDist == -1){
