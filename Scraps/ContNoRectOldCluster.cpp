...
public:
    ...
    Match(Vec4f line1, Vec4f line2, double distance, double ang){
        l1 = line1;
        l2 = line2;
        dist = distance;
        angle = ang;
...
double getAngle(Vec4f line1){
    ...
    angle1 *= (180/ CV_PI);
    if(angle1 < -10) angle1 = 180 + angle1; // All angles should be in range of 0-180 degrees
    return angle1;
}
   ...
   //cout << "Closest Index: " << closest << ",     Distance: " << closestDist << endl;
    ...
    if(whichLine == 2){ // Matches start with rightmost line
        ...
        for(int i = templateLine-1; i >= 0; i--){
            ...
            int xdist = 9999;
            for(int j = 0; j < matches.size(); j++){
                int mxDist = abs( getCenter(matches[j].l1)[0] - getCenter(matches[j].l2)[0]);
                if(mxDist < xdist ){
                    if(matches[j].l1 == templateLines[i]){
                        long lastMatch = bestMatches.size()-1;
    ...
    for( int i = 0; i < matches.size(); i++){
        bool flag = false;
        int myDist = abs( getCenter(matches[i].l1)[1] - getCenter(matches[i].l2)[1]);
        if(myDist < ydist ){
    ...
    for( int i = 0; i < matches.size(); i++){
        ...
        if(myDist < ydist ){
            ...
            if(matches[i].l1 == templateLines[ templateLines.size()-1 ] && !flag){ candidate = matches[i]; // find best match for bottom template line
                ...
                line(src, Point( getCenter(matches[i].l1)[0], getCenter(matches[i].l1)[1]), Point( getCenter(matches[i].l2)[0], getCenter(matches[i].l2)[1]), Scalar(150,0,255), 2);
                
                ydist = myDist;
                //cout << "dist: " << myDist << " less than: " << ydist << endl;
            }
        }
...
vector<Vec4f> cleanLines(vector<Vec4f> lines){
   ...
   //cout << "\n\n\n\n\n";
    //for(int i = 0; i < sortedLines.size(); i++) cout << sortedLines[i] << endl;
   //cout << "\n\n\n\n\n";
    ...
    for(int i = 0; i < k+1; i++){
        ...
        for(int j = 0; j < labels.rows; j++){
            if(labels.at<int>(j) == i){
                ...
                //inlines.push_back( Vec4f(sortedLines[j][0], sortedLines[j][1], sortedLines[j][2], sortedLines[j][3]));
                inlines.push_back( sortedLines[j]);
               //cout << "lines j: " << sortedLines[j] << endl;
            }
        }
       ...
       //cout << "!!!     " << inlines[0]  << "   size: " << inlines.size()  << ", i: " << i << endl;
        Vec4f pushLine = fitBestLine(inlines, centroid);
       //cout << "???     " << pushLine << endl;
...
int main( int argc, char** argv){
    ...
    if(!useRectification){
        ...
        if(selectedLine == 0){
            mid = getCenter(templateLines[0]);
        } else if(selectedLine == 1){
            mid = getCenter(templateLines[1]);
        } else if(selectedLine == 2){
            mid = getCenter(templateLines[2]);
        }
        
       // cout << mid << endl;
       // cout << clickCoords << endl;
        offsetX = -(mid.x - clickCoords.x);
        offsetY = -(mid.y - clickCoords.y);
       // cout << offsetX << ",  " << offsetY << endl << endl;
    }
    
    for( size_t i = 0; i < templateLines.size(); i++ ){
        templateLines[i][0] += offsetX;
        templateLines[i][2] += offsetX;
        templateLines[i][1] += offsetY;
        templateLines[i][3] += offsetY;
    }
    ...
    for(int i = 0; i < templateLines.size(); i++)
    {
        for(int j = 0; j < lines.size(); j++)
        {
            ...
            if( (getAngle(templateLines[i], rectifiedLines[j]) < 70) || (getAngle(templateLines[i], rectifiedLines[j]) > 170)){
                matches.push_back( Match(templateLines[i], lines[j], dist, getAngle(templateLines[i], rectifiedLines[j])));
            }
        }
    }
